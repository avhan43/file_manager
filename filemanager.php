...
define("WEBSITE_BASE_PATH","http://localhost/file_manager/");
...
function get_icon($type, $name=""){
  if (strpos($type, 'excel') !== false) {
    return "<img src='images/ic_excel.png' style='width: 20px'/>";
  }else if(strpos($type, 'audio') !== false){
    return "<img src='images/ic_music.png' style='width: 20px'/>";
  }else if(strpos($type, 'pdf') !== false){
    return "<img src='images/ic_pdf.png' style='width: 20px'/>";
  }else if(strpos($type, 'openxmlformats') !== false){
    if(strpos($name, 'doc') !== false){
      return "<img src='images/ic_word.png' style='width: 20px'/>";
    }else if(strpos($name, 'ppt') !== false){
      return "<img src='images/ic_power_point.png' style='width: 20px'/>";
    }else if(strpos($name, 'xlsx') !== false){
      return "<img src='images/ic_excel.png' style='width: 20px'/>";
    }else{
      return "<img src='images/ic_file.png' style='width: 20px'/>";
    }
  }else if(strpos($type, 'image') !== false){
    return "<img src='images/ic_picture.png' style='width: 20px'/>";
  }else if(strpos($type, 'video') !== false){
    return "<img src='images/ic_video.png' style='width: 20px'/>";
  }else{
    return "<img src='images/ic_file.png' style='width: 20px'/>";
  }
}
...
<head>
  <meta charset="UTF-8">
  <title>Sistem Upload Tugas</title>
...
<body>
  ...
  <div>
    <h1>Sistem Upload Tugas</h1>
  </div>
<div class="input-group mb-3">
<form method="post" enctype="multipart/form-data">
  File: Tambahkan File
  <input type="file" name="file" class="form-control" />
  Folder: <input type="text" name="folder" class="form-control">
  <button type="submit" name="submit" class="btn btn-success mt-3">Submit</button>
</form>
</div>
<br>
<h2><a href="<?Php echo create_url("filemanager.php");?>" class="title ">Files and Folders</a> <?php echo get_current_folder_name();?></h2>
...
<table class="table">
  ...
  <tbody>
      <?php if(in_folder()):?>
        <tr>
          <td colspan="4"><img src='images/ic_folder.png' style='width: 20px'/> &nbsp;
            <a href="<?php echo parent_up_url();?>">...</a></td>
        </tr>
      <?php endif;?>
      <?php foreach(get_all_parent_folders() as $folderObject):?>
        <tr>
          <td><img src='images/ic_folder.png' style='width: 20px'/> &nbsp;
